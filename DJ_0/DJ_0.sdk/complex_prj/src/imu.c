...
void IMU_AHRSupdate(void) {
    ...
    gx = mygetqval[3] * MM_PI/180;
    gy = mygetqval[4] * MM_PI/180;
    gz = mygetqval[5] * MM_PI/180;
    ax = mygetqval[0];
    ay = mygetqval[1];
    az = mygetqval[2];
    mx = mygetqval[6];
    ...
    now = Get_Time_Micros();  //读取时间 单位是ms
    if(now<lastUpdate)
    {
...
{  
    ...
    angles[0] = -atan2(2 * q[1] * q[2] + 2 * q[0] * q[3], -2 * q[2]*q[2] - 2 * q[3] * q[3] + 1)* 180/MM_PI; // yaw        -pi----pi
    angles[1] = -asin(-2 * q[1] * q[3] + 2 * q[0] * q[2])* 180/MM_PI; // pitch    -pi/2    --- pi/2
    angles[2] = atan2(2 * q[2] * q[3] + 2 * q[0] * q[1], -2 * q[1] * q[1] - 2 * q[2] * q[2] + 1)* 180/MM_PI; // roll       -pi-----pi
