...
uint8_t HMC5883_Init(void)  //写地址0X3C 0X48是错的    0X1E是对的
{
    ...
    IIC_Write(HMC58X3_ID, HMC58X3_R_CONFA,0x70,2);
    usleep(5 * 1000); //5ms
    IIC_Write(HMC58X3_ID, HMC58X3_R_CONFB,0xA0,2);
    usleep(5 * 1000); //5ms
    IIC_Write(HMC58X3_ID, HMC58X3_R_MODE,0x00,2);    //这里初始化为0x00 连续模式
    //wait the response of the hmc5883 stabalizes, 6 milliseconds  
    usleep(6 * 1000); //6ms
    //set DOR
    IIC_Write(HMC58X3_ID, HMC58X3_R_CONFA,0x18,2);   //75HZ更新
    usleep(6 * 1000); //6ms
...
void HMC58X3_ReadData(u8 *vbuff) {   
   MPU6050_ReadData(HMC58X3_ID,HMC58X3_R_XM,vbuff,6);   //读取到磁力计数据
}
