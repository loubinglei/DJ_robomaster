...
#define TEST_BTR_SYNCJUMPWIDTH_0		1
#define TEST_BTR_SECOND_TIMESEGMENT_0	7
#define TEST_BTR_FIRST_TIMESEGMENT_0	15  //bps = 100M/(3+1)((15+1)+(7+1)+1)= 1Mbps
#define TEST_BRPR_BAUD_PRESCALAR_0	    3

/************************** Function Prototypes ******************************/
int CanPs_Intr_init_0(INTC *IntcInstPtr, XCanPs *CanInstPtr,
		u16 CanDeviceId, u16 CanIntrId);
void GYRO_RST(void);
int SendFrame_0(XCanPs *InstancePtr);
static void Config_0(XCanPs *InstancePtr);
static void SendHandler_0(void *CallBackRef);
static void RecvHandler_0(void *CallBackRef);
static void ErrorHandler_0(void *CallBackRef, u32 ErrorMask);
static void EventHandler_0(void *CallBackRef, u32 Mask);

static int SetupInterruptSystem_0(INTC *IntcInstancePtr,
				XCanPs *CanInstancePtr,
				u16 CanIntrId);
/************************** Variable Definitions *****************************/


volatile static int LoopbackError_0;	/* Asynchronous error occurred */
volatile static int RecvDone_0;		/* Received a frame */
volatile static int SendDone_0;		/* Frame was sent successfully */
		...
		u16 CanDeviceId, u16 CanIntrId)
{
	...
	SendDone_0 = FALSE;
	RecvDone_0 = FALSE;
	LoopbackError_0 = FALSE;

	/*
	 * Connect to the interrupt controller.
	 */
	Status =  SetupInterruptSystem_0(IntcInstPtr,
					CanInstPtr,
					CanIntrId);
...
static void Config_0(XCanPs *InstancePtr)
{
	 ...
	 */
	XCanPs_SetBaudRatePrescaler(InstancePtr, TEST_BRPR_BAUD_PRESCALAR_0);
	XCanPs_SetBitTiming(InstancePtr, TEST_BTR_SYNCJUMPWIDTH_0,
					TEST_BTR_SECOND_TIMESEGMENT_0,
					TEST_BTR_FIRST_TIMESEGMENT_0);
...
*
******************************************************************************/
static void EventHandler_0(void *CallBackRef, u32 IntrMask)
{
...
void GYRO_RST(void)
{
	...
	TxFrame_0[0] = (u32)XCanPs_CreateIdValue(0x404, 0, 0, 0, 0);
	TxFrame_0[1] = (u32)XCanPs_CreateDlcValue((u32)FRAME_DATA_LENGTH);
	FramePtr = (u8 *)(&TxFrame_0[2]);   // TxFrame has 4 worlds
...
int SendFrame_0(XCanPs *InstancePtr)
{
	...
	Status = XCanPs_Send(InstancePtr, TxFrame_0);
	if (Status != XST_SUCCESS) {
		/*
...
//	int Index;
//	u8 temp;
	Status = XCanPs_Recv(CanPtr, RxFrame_0);
	if (Status != XST_SUCCESS) {
		LoopbackError_0 = TRUE;
		RecvDone_0 = TRUE;
		return;
